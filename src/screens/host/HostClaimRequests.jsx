...
const HostClaimRequests = ({ navigation, route }) => {
  ...
  // New function for bulk approve (direct approval without popup)
  const processBulkApprove = async () => {
    if (selectedClaims.length === 0) return;

    Alert.alert(
      "Approve Selected Claims",
      `Are you sure you want to approve ${selectedClaims.length} selected claim(s)? This action cannot be undone.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Approve",
            ...
            try {
              ...
              const response = await axios.post(
                `https://tambolatime.co.in/public/api/host/games/${gameId}/claims/bulk-process`,
                {
                  claim_ids: selectedClaims,
                  action: "approve",
                  host_response: `Approved ${selectedClaims.length} claims in bulk`
                },
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
              ...
              if (response.data.success && isMountedRef.current) {
                if (!detailModalVisible && !isModalOpenRef.current) {
                  showSnackbar(
                    `${selectedClaims.length} claim(s) approved successfully!`, 
                    'success'
                  );
                }
              ...
              console.log("Error processing bulk approve:", error);
              if (isMountedRef.current && !detailModalVisible && !isModalOpenRef.current) {
                showSnackbar(
                  error.response?.data?.message || "Failed to approve claims",
                  'error'
                );
              }
            } finally {
