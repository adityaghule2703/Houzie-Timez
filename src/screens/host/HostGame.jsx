...
// //         ]}
// //       >
// //         <Text style={styles.ticketText}>🎮 Loading Games...</Text>
// //       </Animated.View>
// //     </LinearGradient>
// //   );
// // };
...
//   const [filteredGames, setFilteredGames] = useState([]);
//   const [error, setError] = useState(null);
//   const [gameStats, setGameStats] = useState({}); // Store stats for completed games
...
//   useEffect(() => {
//     // Fetch stats for completed games when games are loaded
//     if (games.length > 0) {
//       fetchCompletedGamesStats();
//     }
//   }, [games]);
...
//     setRefreshing(true);
//     await fetchGames();
//     await fetchCompletedGamesStats();
//     setRefreshing(false);
//   };
...
//   const fetchCompletedGamesStats = async () => {
...
//         const token = await AsyncStorage.getItem("hostToken");
//         const response = await axios.get(
//           `https://tambolatime.co.in/public/api/host/game/${game.id}/users`,
//           {
//             headers: {
//               Authorization: `Bearer ${token}`,
//               Accept: "application/json",
...
//         if (response.data.success) {
//           statsMap[game.id] = {
//             total_users: response.data.summary.total_users,
//             total_approved_tickets: response.data.summary.total_approved_tickets,
//             total_paid_amount: response.data.summary.total_paid_amount
//           };
//         }
//       } catch (error) {
//         console.log(`Error fetching stats for game ${game.id}:`, error);
//       }
//     }
...
//   const fetchSingleGameStats = async (gameId) => {
//     try {
//       const token = await AsyncStorage.getItem("hostToken");
//       const response = await axios.get(
...
//       }
//     } catch (error) {
//       console.log(`Error fetching stats for game ${gameId}:`, error);
//     }
//   };
...
//     const gameStatsData = gameStats[game.id];
...
//           <View style={styles.detailRow}>
//             {isCompleted && gameStatsData ? (
//               <>
//                 <View style={styles.detailItem}>
//                   <Ionicons name="people-outline" size={16} color="#666" />
//                   <Text style={styles.detailText}>
//                     {gameStatsData.total_users} players 
//                   </Text>
//                 </View>
...
//                   <Text style={styles.detailText}>
//                     {gameStatsData.total_approved_tickets} tickets sold
//                   </Text>
//                 </View>
//               </>
//             ) : (
...
//                       <Ionicons name="layers-outline" size={14} color="#7E57C2" />
//                       <Text style={styles.patternsInlineText}>
//                         {game.selected_patterns.length} patterns
//                       </Text>
//                     </View>
//                   )}
//                 </View>
...
//                   ₹{gameStatsData.total_paid_amount} collected
//                 </Text>
//               </>
//             )}
//           </View>
...
const HostGame = ({ navigation, route }) => {
  ...
  const fetchGames = async (page = 1) => {
    try {
      if (page === currentPage && games.length > 0) {
      ...
      const response = await axios.get(
        `https://tambolatime.co.in/public/api/host/games?page=${page}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: "application/json",
      ...
      if (response.data.success) {
        ...
        const paginationData = {
          current_page: response.data.games.current_page,
          last_page: response.data.games.last_page,
          total: response.data.games.total,
          per_page: response.data.games.per_page
        };
    ...
    return (
      <View style={styles.paginationFooter}>
        ...
        <View style={styles.paginationControls}>
          ...
          >
            <Ionicons name="chevron-back" size={20} color={currentPage === 1 ? "#CCC" : "#FF7675"} />
          ...
          >
            ...
            <Ionicons name="chevron-forward" size={20} color={currentPage === lastPage ? "#CCC" : "#FF7675"} />
          </TouchableOpacity>
        </View>
      </View>
    );
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        <View style={styles.headerContent}>
          <Text style={styles.headerTitle}>Tambola Games</Text>
          <Text style={styles.headerSubtitle}>
            {searchQuery ? `${filteredGames.length} games found` : `${totalGames} total games`}
          </Text>
        </View>
        <View style={styles.headerActions}>
          <TouchableOpacity style={styles.refreshButton} onPress={onRefresh}>
      ...
      <FlatList
          ...
          <>
            {!searchQuery && filteredGames.length > 0 && (
              <View style={styles.statsContainer}>
                <View style={styles.statsCard}>
                  <Ionicons name="calendar-outline" size={24} color="#FF7675" />
                  <Text style={styles.statsCount}>{totalGames}</Text>
                  <Text style={styles.statsLabel}>Total Games</Text>
                </View>
            ...
            <View style={styles.listHeader}>
              <Text style={styles.listTitle}>
                {searchQuery ? "Search Results" : "Your Games"}
              </Text>
              <TouchableOpacity
                style={styles.patternsButton}
                onPress={() => navigation.navigate("HostGamePatterns")}
        ...
        ListFooterComponent={
          <>
            ...
            {filteredGames.length > 0 && renderPaginationFooter()}
          </>
        }
      />
    </SafeAreaView>
