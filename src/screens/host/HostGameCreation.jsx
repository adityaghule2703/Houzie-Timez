...
const HostGameCreation = ({ navigation, route }) => {
  ...
  const saveInlineReward = useCallback(() => {
    // Validate current pattern rewards before saving
    const currentReward = patternRewards.find(r => r.pattern_id === editingReward);
    if (currentReward) {
      const errors = {};
      if (!currentReward.amount || parseFloat(currentReward.amount) <= 0) {
        errors.amount = 'Amount must be > 0';
      }
      if (!currentReward.reward_count || parseInt(currentReward.reward_count) <= 0) {
        errors.reward_count = 'Count must be > 0';
      }
      if (!currentReward.min_tickets_required || parseInt(currentReward.min_tickets_required) <= 0) {
        errors.min_tickets_required = 'Min tickets must be > 0';
      }
      
      if (Object.keys(errors).length > 0) {
        setRewardErrors(prev => ({
          ...prev,
          [editingReward]: errors
        }));
        showToast('Please fill all reward fields with values greater than 0', 'error');
        return;
      }
    }
    setEditingReward(null);
  }, [editingReward, patternRewards]);
              ...
              <View style={styles.selectedPatternsSection}>
                ...
                <View style={styles.selectedPatternsList}>
                  {selectedPatterns.map((pattern) => {
                    ...
                    const iconName = getPatternIcon(pattern);
                    const color = getPatternColor(pattern.logic_type);
                    const errors = rewardErrors[pattern.id] || {};
                        ...
                        <View style={styles.selectedPatternHeader}>
                          ...
                          {!isEditing ? (
                            ...
                            >
                              <Ionicons name="pencil" size={12} color="#4CAF50" />
                              <Text style={styles.configureRewardText}>
                                {reward?.amount && parseFloat(reward.amount) > 0 ? `₹${reward.amount}` : 'Set'}
                              </Text>
                            </TouchableOpacity>
                          ) : (
                            <TouchableOpacity
                            ...
                            {isEditing ? (
                              <>
                                <View style={styles.inlineInputGroup}>
                                  <Text style={styles.inlineInputLabel}>Reward Name *</Text>
                                  <TextInput
                                    style={[styles.inlineInput, errors.reward_name && styles.inlineInputError]}
                                    value={reward.reward_name}
                                    onChangeText={(value) => updatePatternReward(pattern.id, 'reward_name', value)}
                                    placeholder="Reward name"
                                  />
                                  {errors.reward_name && (
                                    <Text style={styles.errorText}>{errors.reward_name}</Text>
                                  )}
                                </View>
                                ...
                                <View style={styles.inlineRewardDetails}>
                                  <View style={styles.inlineRewardDetail}>
                                    <Text style={styles.inlineRewardLabel}>Amount (₹) *</Text>
                                    <TextInput
                                      style={[styles.inlineRewardInput, errors.amount && styles.inlineInputError]}
                                      value={reward.amount}
                                      onChangeText={(value) => updatePatternReward(pattern.id, 'amount', value)}
                                      keyboardType="numeric"
                                      placeholder="Amount"
                                  ...
                                  <View style={styles.inlineRewardDetail}>
                                    <Text style={styles.inlineRewardLabel}>Reward Count *</Text>
                                    <TextInput
                                      style={[styles.inlineRewardInput, errors.reward_count && styles.inlineInputError]}
                                      value={reward.reward_count}
                                      onChangeText={(value) => updatePatternReward(pattern.id, 'reward_count', value)}
                                      keyboardType="numeric"
                                      placeholder="Count"
                                    />
                                    {errors.reward_count && (
                                      <Text style={styles.errorText}>{errors.reward_count}</Text>
                                    )}
                                  </View>
                                  ...
                                  <View style={styles.inlineRewardDetail}>
                                    <Text style={styles.inlineRewardLabel}>Min Tickets *</Text>
                                    <TextInput
                                      style={[styles.inlineRewardInput, errors.min_tickets_required && styles.inlineInputError]}
                                      value={reward.min_tickets_required}
                                      onChangeText={(value) => updatePatternReward(pattern.id, 'min_tickets_required', value)}
                                      keyboardType="numeric"
                                      placeholder="Min tickets"
                                    />
                                    {errors.min_tickets_required && (
                                      <Text style={styles.errorText}>{errors.min_tickets_required}</Text>
                                    )}
                                  </View>
                                </View>
                                
                                <View style={styles.inlineInputGroup}>
                                  <Text style={styles.inlineInputLabel}>Description (Optional)</Text>
                                  <TextInput
                                    style={[styles.inlineInput, styles.inlineTextArea]}
                                    value={reward.description}
                                    onChangeText={(value) => updatePatternReward(pattern.id, 'description', value)}
