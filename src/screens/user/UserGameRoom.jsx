...
const PATTERN_SEQUENCE = [
  ...
  { keywords: ['bullseye', 'bull eye', 'center', 'centre'] },
  { keywords: ['new age', 'newage', 'young age', '1-45'] },
  { keywords: ['old age', 'oldage', 'senior', '46-90'] },
...
const UserGameRoom = ({ navigation, route }) => {
  ...
  const UNREAD_KEY = `unread_chat_count_${gameId}`;
    ...
    while (markingQueueRef.current.length > 0) {
      ...
      try {
        ...
        retryTimeoutsRef.current[retryKey] = setTimeout(() => {
          ...
          markingQueueRef.current.unshift(retryMarking);
  ...
  const checkPatternCompletedForTicket = (pattern, ticket) => {
    ...
    const patternName = (pattern.reward_name || '').toLowerCase()
      .replace(/ /g, '_').replace('_prize', '').replace(' prize', '');
    ...
    if (patternName.includes('early_five') || patternName.includes('early five')) return marked.length >= 5;
    if (patternName.includes('four_corners') || patternName.includes('four corners')) {
      ...
      if (firstRowCells.length >= 2) { corners.push(firstRowCells[0].number); corners.push(firstRowCells[firstRowCells.length - 1].number); }
      const lastRowCells = processedData[2].filter(c => c?.number != null);
      if (lastRowCells.length >= 2) { corners.push(lastRowCells[0].number); corners.push(lastRowCells[lastRowCells.length - 1].number); }
    ...
    if (patternName.includes('bamboo')) {
      ...
      for (let row = 0; row < 3; row++) {
        ...
        if (rowCells.length >= 3) bambooNums.push(rowCells[2].number);
      }
      return bambooNums.length === 3 && bambooNums.every(n => marked.includes(n));
    }
    if (patternName.includes('bullseye') || patternName.includes('bull eye') || patternName.includes('center') || patternName.includes('centre')) {
      const secondRowCells = processedData[1].filter(c => c?.number != null);
      if (secondRowCells.length >= 5) {
        const centerIndex = Math.floor(secondRowCells.length / 2);
        const centerNumber = secondRowCells[centerIndex].number;
        return marked.includes(centerNumber);
      }
      return false;
    }
    if (patternName.includes('new_age') || patternName.includes('newage') || patternName.includes('young_age') || patternName === '1-45') {
      const numbersInRange = allNums.filter(n => n >= 1 && n <= 45);
      if (numbersInRange.length === 0) return false;
      return numbersInRange.every(n => marked.includes(n));
    }
    if (patternName.includes('old_age') || patternName.includes('oldage') || patternName.includes('senior') || patternName === '46-90') {
      const numbersInRange = allNums.filter(n => n >= 46 && n <= 90);
      if (numbersInRange.length === 0) return false;
      return numbersInRange.every(n => marked.includes(n));
    }
    return false;
  ...
  const joinChat = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        navigation.navigate('UserLiveChat', { gameId, gameName });
      }
    } catch (error) {
      ...
      if (!userName || !patternName || userName === 'Someone' || patternName === 'pattern') {
        fetchClaimDetails(claim.id).then(completeClaim => {
          if (completeClaim) {
            ...
            if (type === 'claim_approved') addToTtsQueue(`Congratulations! ${completeClaim.user_name} has won for completing the ${cleanName}!`);
        ...
        if (type === 'claim_approved') addToTtsQueue(`Congratulations! ${userName} has won for completing the ${cleanPattern}!`);
    ...
    switch (patternName) {
      case 'bamboo':
        for (let row = 0; row < 3; row++) {
          ...
          for (let col = 0; col < 9; col++) { 
            ...
            if (cell?.number != null) nonEmptyCells.push({ row, col, cell }); 
          }
      ...
      case 'bottom_line':
        for (let col = 0; col < 9; col++) { 
          ...
          if (cell?.number != null) cells.push({ row: 2, col }); 
        }
        break;
      case 'breakfast':
        for (let row = 0; row < 3; row++) 
          for (let col = 0; col < 9; col++) { 
            const cell = processedData[row][col]; 
            if (cell?.number != null && cell.number >= 1 && cell.number <= 30) cells.push({ row, col }); 
          }
        break;
      case 'dinner':
        for (let row = 0; row < 3; row++) 
          for (let col = 0; col < 9; col++) { 
            const cell = processedData[row][col]; 
            if (cell?.number != null && cell.number >= 61 && cell.number <= 90) cells.push({ row, col }); 
          }
        break;
      case 'early_five':
        ...
        for (let row = 0; row < 3; row++) 
          for (let col = 0; col < 9; col++) { 
            const cell = processedData[row][col]; 
            if (cell?.number != null) ticketNumberPositions[cell.number] = { row, col }; 
          }
        let foundCount = 0;
        for (const calledNumber of calledNumbers) {
          if (ticketNumberPositions[calledNumber] && foundCount < 5) { 
            cells.push(ticketNumberPositions[calledNumber]); 
            foundCount++; 
          }
      ...
      case 'four_corners':
        ...
        for (let col = 0; col < 9; col++) { 
          ...
          if (cell?.number != null) firstRowCells.push({ row: 0, col, cell }); 
        }
        ...
        for (let col = 0; col < 9; col++) { 
          const cell = processedData[2][col]; 
          if (cell?.number != null) lastRowCells.push({ row: 2, col, cell }); 
        }
      ...
      case 'full_house':
        for (let row = 0; row < 3; row++) 
          for (let col = 0; col < 9; col++) { 
            const cell = processedData[row][col]; 
            if (cell?.number != null) cells.push({ row, col }); 
          }
        break;
      case 'lunch':
        for (let row = 0; row < 3; row++) 
          for (let col = 0; col < 9; col++) { 
            const cell = processedData[row][col]; 
            if (cell?.number != null && cell.number >= 31 && cell.number <= 60) cells.push({ row, col }); 
          }
        break;
      case 'middle_line':
        for (let col = 0; col < 9; col++) { 
          const cell = processedData[1][col]; 
          if (cell?.number != null) cells.push({ row: 1, col }); 
        }
        break;
      case 'top_line':
        for (let col = 0; col < 9; col++) { 
          const cell = processedData[0][col]; 
          if (cell?.number != null) cells.push({ row: 0, col }); 
        }
        break;
      case 'bullseye':
      case 'bull_eye':
      ...
      case 'centre':
        ...
        for (let col = 0; col < 9; col++) {
          const cell = processedData[1][col];
          if (cell?.number != null) {
            secondRowCells.push({ row: 1, col });
          }
        }
        if (secondRowCells.length >= 5) {
          const centerIndex = Math.floor(secondRowCells.length / 2);
          cells.push(secondRowCells[centerIndex]);
        }
        break;
      case 'new_age':
      case 'newage':
      ...
      case '1-45':
        for (let row = 0; row < 3; row++) {
          for (let col = 0; col < 9; col++) {
            const cell = processedData[row][col];
            if (cell?.number != null && cell.number >= 1 && cell.number <= 45) {
              cells.push({ row, col });
            }
          }
        }
      ...
      case '46-90':
        for (let row = 0; row < 3; row++) {
          for (let col = 0; col < 9; col++) {
            const cell = processedData[row][col];
            if (cell?.number != null && cell.number >= 46 && cell.number <= 90) {
              cells.push({ row, col });
            }
          }
        }
  ...
  const getPatternDescription = (patternName) => {
    ...
    const descriptions = {
      ...
      'bullseye': 'Mark the center number on your ticket',
      'new_age': 'Mark all numbers from 1-45 on your ticket',
      'old_age': 'Mark all numbers from 46-90 on your ticket',
    };
 ...
 const fetchGamePatternsForViewing = async () => {
      ...
      const chatChannel = await pusher.subscribe({
        ...
        onEvent: (event) => {
          ...
          if (event.eventName === 'new.message') {
            ...
            if (currentRouteName !== 'UserLiveChat') {
  ...
  const handleNumberClick = async (ticketId, cellNumber, isCurrentlyMarked) => {
    ...
    updateTicketState(ticketId, cellNumber, newMarkedState);
    
    if (!pendingMarkingsRef.current[ticketId]) {
      pendingMarkingsRef.current[ticketId] = {};
    }
    pendingMarkingsRef.current[ticketId][cellNumber] = newMarkedState;
    setPendingMarkings({ ...pendingMarkingsRef.current });
    await savePendingMarkings();
...
const renderPatternsModal = () => {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <SuccessClaimModal visible={showSuccessModal} patternName={successPatternName} onClose={() => setShowSuccessModal(false)} />
      {renderPatternsModal()}
        ...
        <TouchableOpacity style={styles.floatingChatButton} onPress={joinChat} activeOpacity={0.9}>
          <LinearGradient colors={COLORS.primaryGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.chatButtonGradient}>
            ...
            <View style={styles.chatButtonContent}>
              ...
              {unreadChatCount > 0 ? (
                <LinearGradient colors={['#FF4444', '#CC0000']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.unreadBadge}>
...
const styles = StyleSheet.create({
  ...
  unreadBadge: { position: 'absolute', top: -6, right: -6, borderRadius: 8, minWidth: 16, height: 16, justifyContent: 'center', alignItems: 'center', borderWidth: 1.5, borderColor: COLORS.surface },
  unreadBadgeText: { color: COLORS.surface, fontSize: 9, fontWeight: '900', paddingHorizontal: 3 },
