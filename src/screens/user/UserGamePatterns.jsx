...
const CELL_SIZE = (TICKET_WIDTH - TICKET_PADDING * 2 - CELL_MARGIN * 2 * NUM_COLUMNS) / NUM_COLUMNS;

// Row colors for ticket
...
// Process ticket data to ensure consistent format
...
const UserGamePatterns = () => {
  ...
  const MiniTicketGrid = ({ pattern }) => {
    ...
    const processedData = useMemo(() => processTicketData(ticketNumbers), [ticketNumbers]);
    ...
    return (
      ...
      >
        ...
        {processedData.map((row, rowIndex) => (
          <View 
            key={`mini-row-${pattern.id}-${rowIndex}`} 
            style={[
              styles.miniRow,
              { 
  ...
  const FullTicketGrid = ({ pattern }) => {
    ...
    const dynamicCellSize = useMemo(() => {
      const availableWidth = TICKET_WIDTH - (TICKET_PADDING * 2) - (CELL_MARGIN * 2 * NUM_COLUMNS);
    ...
    return (
      <LinearGradient
        ...
        style={[styles.fullTicket, { width: TICKET_WIDTH }]}
      >
        <LinearGradient
          colors={COLORS.prizeGradient}
          start={{ x: 0, y: 0 }}
        ...
        {processedData.map((row, rowIndex) => (
          <View 
            key={`full-row-${pattern.id}-${rowIndex}`} 
            style={[
              styles.fullRow,
              { 
                backgroundColor: rowIndex % 2 === 0 ? ROW_COLOR_1 : ROW_COLOR_2,
          ...
          >
            {row.map((cell, colIndex) => {
              ...
              const isPatternPosition = patternGrid && patternGrid[rowIndex] && patternGrid[rowIndex][colIndex];
              ...
              return (
                <View
                  ...
                  style={[
                    ...
                    isPatternPosition && styles.fullCellPattern,
                  ]}
                >
                  {!isEmpty && (
                    <>
                      <Text style={[
                        ...
                        { fontSize: dynamicCellSize * 0.35 },
                        isPatternPosition && styles.fullCellNumberPattern,
                      ]}>
                        {cell}
                      </Text>
                      {isPatternPosition && (
                        <LinearGradient
                          ...
                          style={[styles.positionIndicator, { width: dynamicCellSize * 0.3, height: dynamicCellSize * 0.3 }]}
                        >
                          <Text style={[styles.positionIndicatorText, { fontSize: dynamicCellSize * 0.2 }]}>
                            {getPositionNumber(row, colIndex)}
                          </Text>
                        </LinearGradient>
                      )}
                    </>
...
const styles = StyleSheet.create({
  ...
  miniCellWithNumber: {
    backgroundColor: FILLED_CELL_BG,
  },
  miniCellPattern: {
