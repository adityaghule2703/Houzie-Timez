...
const UserGameWinners = ({ navigation, route }) => {
  ...
  const formatTimeSinceApproval = (approvedAt) => {
    if (!approvedAt) return 'Just now';
    
    const approvedTime = new Date(approvedAt);
    const now = new Date();
    const diffInSeconds = Math.floor((now - approvedTime) / 1000);
  ...
  const renderWinnersList = () => {
    ...
    // Filter winners by pattern_name instead of game_pattern_id
    const filteredWinners = winnersData.winners.filter(
      winner => winner.pattern_name === selectedPattern.pattern_name
    );
        ...
        renderItem={({ item: winner, index }) => (
          <View key={winner.id} style={styles.winnerCard}>
            ...
            <View style={styles.winnerFooter}>
              <View style={styles.winTimeBadge}>
                <Ionicons name="time" size={14} color={TEXT_LIGHT} />
                <Text style={styles.winTimeText}>
                  {formatTimeSinceApproval(winner.approved_at)}
                </Text>
              </View>
            ...
            {selectedTicket && (
              <>
                  ...
                  <View style={styles.modalUserInfo}>
                    <View style={styles.modalAvatar}>
                      {selectedTicket.profile_image ? (
                        <Image 
                          source={{ uri: selectedTicket.profile_image }} 
                ...
                <View style={styles.modalFooter}>
                  <View style={styles.modalWinTime}>
                    <Ionicons name="time" size={14} color={TEXT_LIGHT} />
                    <Text style={styles.modalWinTimeText}>
                      Won {formatTimeSinceApproval(selectedTicket.approved_at)}
                    </Text>
                  </View>
                  <TouchableOpacity
                    style={styles.closeModalButton}
