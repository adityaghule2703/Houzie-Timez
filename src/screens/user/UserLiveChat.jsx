...
//       type: data.message_type === 'media' ? 'voice' : (data.type || 'text'),
...
//       console.log(`[Chat ${gameId}] Adding new message from ${newMessage.sender_name}`);
...
//       type: 'system',
//       message: `${data.user?.name || 'Someone'} joined the chat`,
//       timestamp: data.timestamp,
...
//       type: 'system',
//       message: `${data.user?.name || 'Someone'} left the chat`,
//       timestamp: data.timestamp,
//       created_at: data.timestamp,
//       sender_type: 'system',
//     };
...
//     if (data.muted_user?.id === currentUserId) {
//       fetchMuteStatus();
//       Alert.alert("Muted", "You have been muted in this chat");
//     }
//   };
...
//       // Subscribe to chat channel - matching backend channel name
...
//         // Initialize Pusher after joining
//         await initializePusher();
...
const UserLiveChat = ({ navigation, route }) => {
      ...
      if (response.data.success && isMounted.current) {
        ...
        const newMessages = filteredMessages.map(msg => {
          ...
          if (isImage && msg.attachment) {
            imageUrl = getFullImageUrl(msg.attachment);
          }
          
          return {
            id: msg.id.toString(),
            type: isVoice ? 'voice' : (isImage ? 'image' : (messageType || 'text')),
            message: msg.message || '',
            timestamp: msg.timestamp,
            created_at: msg.created_at,
            sender_id: msg.sender?.id,
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const imageUrl = isImage && serverMessage.attachment ? getFullImageUrl(serverMessage.attachment) : null;
        ...
        const newMsg = {
          ...
          sender_id: serverMessage.sender?.id || currentUserId,
          sender_name: serverMessage.sender?.name || currentUserName,
          sender_type: serverMessage.sender?.type || "user",
  ...
  const sendImage = async (uri, filename) => {
    ...
    try {
      ...
      if (response.data.success) {
        const serverMessage = response.data.data;
        const imageUrl = serverMessage.attachment ? getFullImageUrl(serverMessage.attachment) : null;
  ...
  const handleNewMessage = (data) => {
    ...
    const imageUrl = isImage && data.attachment ? getFullImageUrl(data.attachment) : null;
    
    const newMessage = {
      id: data.id ? data.id.toString() : `${Date.now()}-${Math.random()}`,
      type: isImage ? 'image' : (messageType === 'media' ? 'voice' : (messageType || 'text')),
      message: data.message || '',
      timestamp: data.timestamp,
      created_at: data.created_at,
      sender_id: data.sender?.id,
      ...
      if (message.attachment_url) {
        ...
        if (message.attachment.url) {
          if (message.attachment.url.startsWith('http')) {
            ...
            imageUrl = `https://tambolatime.co.in/public${message.attachment.url}`;
          }
        } else if (message.attachment.path) {
          if (message.attachment.path.startsWith('http')) {
            imageUrl = message.attachment.path;
          } else {
            imageUrl = `https://tambolatime.co.in/public${message.attachment.path}`;
          }
        }
      }
    }
              ...
              <View style={styles.mediaContainer}>
                {imageUrl ? (
                  <Image
                    ...
                    style={styles.mediaImage}
                    resizeMode="cover"
                    onError={(e) => console.log(`[Chat ${gameId}] Image load error:`, e.nativeEvent.error, imageUrl)}
                  />
                ) : (
                ...
                {isPending && !imageUrl?.startsWith('http') && (
              ...
              <View style={styles.mediaContainer}>
                {imageUrl ? (
                  <Image
                    ...
                    style={styles.mediaImage}
                    resizeMode="cover"
                    onError={(e) => console.log(`[Chat ${gameId}] Image load error from other:`, e.nativeEvent.error, imageUrl)}
                  />
                ) : (
                  <View style={styles.imagePlaceholder}>
                    <Ionicons name="image-outline" size={40} color="#999" />
      ...
      <View style={[
        ...
        <View style={styles.inputWrapper}>
          ...
          <TouchableOpacity
            style={[
              styles.sendButton,
              (!newMessage.trim() || isMuted) && styles.sendButtonDisabled
            ]}
            onPress={sendMessage}
